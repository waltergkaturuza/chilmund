          ...
          <div className="grid items-stretch gap-10 lg:grid-cols-12 lg:gap-8 xl:gap-10">
            <div className="flex max-w-3xl flex-col justify-center lg:col-span-6 xl:max-w-none">
              {children ||
                (richText && (
                  <RichText
                    className="mb-0 prose-headings:font-bold prose-headings:tracking-tight prose-headings:text-white prose-p:text-pretty prose-p:leading-relaxed prose-p:text-white/85 prose-p:text-sm prose-strong:text-white prose-a:text-blue-500 prose-a:no-underline hover:prose-a:underline [&_h1]:text-justify [&_h1]:[text-align-last:left] [&_h1]:text-blue-400 [&_h1]:leading-[1.08] [&_h1]:text-[clamp(1.1rem,5vw,2.6rem)] [&_h2]:text-white/95 [&_h2]:text-[clamp(1.4rem,5vw,1.9rem)]"
                    data={richText}
                    enableGutter={false}
                  />
                ))}
